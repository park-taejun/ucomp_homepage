...
#=====================================================================
	...
	$use_tf_linktype					= $_POST['use_tf_linktype']!=''?$_POST['use_tf_linktype']:$_GET['use_tf_linktype'];
...
#====================================================================
	if ($mode == "I") {
		...
		$new_bb_no =  insertPopup($conn, $use_tf_date, $use_tf_linktype, $size_w, $size_h, $top, $left, $scrollbars, $s_date, $s_time, $e_date, $e_time, $title, $contents, $use_tf, $reg_date);
	...
	if ($mode == "U") {
		...
		$result = updatePopup($conn, $use_tf_date, $use_tf_linktype, $size_w, $size_h, $top, $left, $scrollbars, $s_date, $s_time, $e_date, $e_time, $title, $contents, $use_tf, $pop_no, $reg_date);
		$result = true;
	}
	...
	if ($mode == "S") {
		...
		$rs_link_type				= trim($arr_rs[0]["CATE_02"]);	//기간사용여부
...
function js_save() {
	...
	if (document.frm.linkType == null) {
		//alert(document.frm.rd_use_tf);
	} else {
		if (frm.linkType[0].checked == true) {
			frm.use_tf_linktype.value = "Y";
		} else {
			frm.use_tf_linktype.value = "N";
		}
	}
...
<input type="hidden" name="reply_state" value="">
			...
			<table summary="이곳에서 <?=$p_menu_name?>을 입력하실 수 있습니다" class="bbsWrite">
				...
				<tr>
					...
					<td colspan="3">
						<input type="radio" class="radio" name="linkType" value="Y" <? if (($rs_link_type =="Y") || ($rs_link_type =="")) echo "checked"; ?>/> 팝업
						&nbsp;&nbsp;&nbsp;<input type="radio" class="radio" name="linkType" value="N" <?if($rs_link_type=="N"){?> checked<?}?>/> 레이어
						<input type="hidden" name="use_tf_linktype" value="<?= $rs_link_type ?>">
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="s_time" value="<?=$rs_s_time?>" maxlength="8" <? if($rs_date_use_tf == "N"){echo "style='width:60px;color:#b8b8b8;'";}else{echo "style='width:60px;color:#000000;'";} ?>>
						...
						<input type="text" name="e_time" value="<?=$rs_e_time?>" maxlength="8" <? if($rs_date_use_tf == "N"){echo "style='width:60px;color:#b8b8b8;'";}else{echo "style='width:60px;color:#000000;'";} ?>>&nbsp;&nbsp;&nbsp;&nbsp; 예) 2015-01-01 17:50:00
					</td>
				</tr>
