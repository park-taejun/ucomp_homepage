...
document.write('.btn     { width: 22px; height: 22px; border: 1px solid white; margin: 0; padding: 0;background:white; }\n');
//document.write(' P { MARGIN-TOP: 1px; MARGIN-BOTTOM: 1px; }\n');
document.write('.btnOver { width: 22px; height: 22px; border: 1px outset;background:white; }\n');
document.write('.btnDown { width: 22px; height: 22px; border: 1px white ; background-color: buttonhighlight; }\n');
...
this.width =  "auto";
this.height = "auto";
this.bodyStyle = 'background-color: #FFFFFF; font-family: "돋움"; font-size: x-small;';
...
//    ['HorizontalRule','Createlink','InsertImage','InsertTable','htmlmode','separator'],
    ['Createlink','InsertImage','InsertTable','htmlmode'],
//  ['custom1'],
...
this.btnList = {
    ...
    "custom1":           ['custom1',         '이미지올림',  'editor_action(this.id)',  'ed_custom.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 bgcolor="white" style="padding: 1 0 0 2" width=' + config.width + ' unselectable="on"><tr><td>\n'
  + toolbar
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
...
// 게시판 경로(이미지 업로드 경로에 사용)
var board_type = document.getElementById("for_editor_board_type").value;
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
       ...
       showModalDialog(_editor_url + "popups/insert_image.html?board_type="+board_type, editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }
...
function editor_setmode(objname, mode) {
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
    ...
    html += 'P { MARGIN-TOP: 1px; MARGIN-BOTTOM: 1px; } \n'; //추가 스타일
    for (var i in config.fontstyles) {
      var fontstyle = config.fontstyles[i];
    ...
    html += '</style>\n'
      + '</head>\n'//스타일 추가함 가로스크롤 안생기게하기위해, 뷰페이지와 비슷하게 보이기위해 뷰페이지 td에 있는 스타일과 같게함
      + '<body style="line-height:180%;font: 돋움;font-size:12px;font-color: #333333; WORD-BREAK: break-all;" contenteditable="true" topmargin=1 leftmargin=1'
