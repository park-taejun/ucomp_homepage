...
function sendMail($EMAIL, $NAME, $SUBJECT, $CONTENT, $mailto ){ //비밀번호찾기 메일, 스마트데이 출퇴근 메일
...
function sendMailSmartday($EMAIL, $NAME, $SUBJECT, $CONTENT, $mailto, $cc){ //스마트데이 출퇴근 메일(참조 포함)
...
function multiupload($filearray, $cnt, $targetdir, $max_size = 2 /* MByte */, $allowext) {
	...
	echo "////multiupload 02".$filearray['name'][$cnt]."<br>";
...
//다중 업로드 재정의
function multiupload_1($filearray, $cnt, $targetdir, $max_size = 2 /* MByte */, $allowext) {
