<?
	...
	if (!empty($qry_str)) {
		...
		$query = "select count(*) from tb_other_board where BoardId = '".$BoardId."' ".$que;
		$query2 = "select * from tb_other_board where BoardId = '".$BoardId."' ".$que. " order by SeqNo desc";

	} else {
		$query = "select count(*) from tb_other_board where BoardId = '".$BoardId."'";
		$query2 = "select * from tb_other_board where BoardId = '".$BoardId."' order by SeqNo desc";
	}
...
function onView(id) {
	...
	document.frmSearch.action= "board_view.php";
	document.frmSearch.submit();
}

function goIn() {
	document.frmSearch.action= "board_input.php";
	document.frmSearch.submit();
}
...
function goDel() {
	...
	if ( bDelOK ==true ) {
		...
		document.frmSearch.action = "board_db.php";
		document.frmSearch.submit();
	}
	else {
		return;
...
<!-- Site Map Section End -->
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
				...
				<tr>
					...
					<td width="99%">
<!-- Form --->					
					<FORM name="frmSearch" method="post" action="board_list.php">
						...
						<table cellpadding="0" cellspacing="1" border="0" width="95%"> 
							<tr>
								<td height="2" bgcolor="#000000" colspan="6"></td>
							</tr>
							...
							<tr>
								<td height="1" bgcolor="#000000" colspan="6"></td>
							</tr>
							<?
							...
							<tr align="center" height="25" onmouseover="this.style.background='#E9E9E9';" onmouseout="this.style.background='White';">
								...
								<? if ($obj->FileRealName01 != "") {?>
								<td><a href="/upload/down_file_func.php?BoardId=<?echo $BoardId?>&id=<?echo $obj->SeqNo?>"><img src="../images/icon_filedown.gif" border="0"></a>&nbsp;&nbsp;<?echo $obj->FileRealName01?></td>
								<? } else {?>
								<td>&nbsp;</td>
								<? }?>
							...
							<tr>
								<td height="1" bgcolor="#C4C4C4" colspan="6"></td>
							</tr>							
							<?
										}
									}
							...
							?>	
							<tr>
								<td height="30" align="center" colspan="9"><strong>등록된 게시물이 없습니다.</strong></td>
							</tr>
							<?
								}
							?>
