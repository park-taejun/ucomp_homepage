...
<?
# =============================================================================
# File Name    : pop_group_order_dml.php
# Modlue       : 
# Writer       : Park Chan Ho 
...
#====================================================================
	...
	for ($k = 0; $k < $row_cnt; $k++) {
		...
		if (strlen($m_level) == 0) {
			$page_level = "GROUP_SEQ01";
		}

		if (strlen($m_level) == 3) {
			$page_level = "GROUP_SEQ02";
		}

		if (strlen($m_level) == 6) {
			$page_level = "GROUP_SEQ03";
		}

		if (strlen($m_level) == 9) {
			$page_level = "GROUP_SEQ04";
		}

		if (strlen($m_level) == 12) {
			$page_level = "GROUP_SEQ05";
		}

		$str_seq = "00".$i;
		$str_seq = substr($str_seq, -3);

		$temp_page_no =  $arr_page_no[$k];

		$temp_page_no = "(" . str_replace("^",",", $temp_page_no) . ")";

		$result = updateGroupOrder($conn, $temp_page_no, $page_level, $str_seq);
...
<!--
	function init() {
		//group_kind=parent.opener.document.frm.group_kind.value;
		parent.opener.document.location = "group_list.php?sel_group_kind=<?=$group_kind?>";
	}
//-->
</script>
