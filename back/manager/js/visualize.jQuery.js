...
$.fn.visualize = function(options, container){
	return $(this).each(function(){
		...
		var o = $.extend({
			...
			pieLabelPos: 'inside',
			lineWeight: 0.8, //for line and area - stroke weight
			barGroupMargin: 10,
		...
		// 그래프가 잘리는 현상으로 인해 width값을 강제로 적용 시켜 준다.
		//o.width = parseFloat(o.width);
		o.width = 1050;
		
		//canvas 사이즈 조절(9등급 셋팅시 값이 전체가 잘 나오기 위해)
		...
		function scrapeTable(){
			...
			var tableData = {
				dataGroups: function(){
					...
					if(o.parseDirection == 'x'){
						self.find('tr:gt(0)').each(function(i){
							dataGroups[i] = {};
							dataGroups[i].points = [];
							dataGroups[i].points2 = [];
							dataGroups[i].color = colors[i];
							if(textColors[i]){ dataGroups[i].textColor = textColors[i]; }
							$(this).find('td').each(function(){
								dataGroups[i].points.push( parseFloat($(this).text()) );
								// 그래프를 거꾸로 그리기위해 points2에 9 - 등급값을 넣어준다
								dataGroups[i].points2.push( 9 - parseFloat($(this).text()) );
							});
						});
					}
					else {
				...
				topValue: function(){
						...
						// topValue 값으로 표를 그려 주므로 9등급 까지 표현
						topValue = 9;
						return topValue;
				},
		...
		var createChart = {
			...
			line: function(area){
				...
				//write X labels
				//x 라벨의 앞 뒤에 공백 추가를 위해 xLabels.length에 2를 더한다.
				...
				$.each(xLabels, function(i){ 
					var thisLi = $('<li><span>'+this+'</span></li>')
						...
						.css('left', xInterval * (i+1))
						.appendTo(xlabelsUL);						
					var label = thisLi.find('span:not(.line)');
					var leftOffset = label.width()/-2;
					//alert("leftOffset : "+ leftOffset);
					// 좌측 공백을 위해 leftOffset 을 시작점으로 한다.
					if(i == 0){ leftOffset =  0;}
				...
				$.each(yLabels, function(i){  
					// li를 위에서 부터 붙이기 위해 css 속성을 top으로 변경
					var thisLi = $('<li><span>'+this+'</span></li>')
						.prepend('<span class="line"  />')
						.css('top',liBottom*i) //.css('bottom',liBottom*i)
						.prependTo(ylabelsUL);
					var label = thisLi.find('span:not(.line)');
				...
				//xInterval 만큼 이동한 상태에서 그래프 그리기 시작
				//ctx.translate(0,zeroLoc);
				ctx.translate(xInterval,zeroLoc);
				//iterate and draw
				$.each(dataGroups,function(h){
					...
					// 그래프를 뒤집기 위해 담아둔 points2값을 사용한다.
					var points = this.points2;
					var integer = 0;
