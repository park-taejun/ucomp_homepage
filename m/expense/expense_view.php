...
<script type="text/javascript">
	...
	function js_delete() {
		...
		if (bDelOK==true) {
			...
			request.done(function(msg) {
				...
				if (msg == "T") {
					alert('삭제 되었습니다.');
					document.location = "/m/expense/expense_list.php";
					return;
				} 
			});
	...
	function js_mail(adm, adm_no) {
		...
		if (bDelOK==true) {
			...
			var request = $.ajax({
				url:"/m/expense/expense_mail.php",
				type:"POST",
				data:{mode:mode
						 ,email:email
						 ,leader_no:leader_no 
									...
									<div class="board-util">
										...
										<div class="button-display module-a style-a type-d">
											<span class="button-area">
												<a href="javascript:js_mail('<?=selectAdminEmail2($conn, $rs_va_user)?>', <?=$rs_va_state_pos?>);" class="btn module-c style-b type-line accent-02 large flex"><span class="btn-text">승인요청 메일발송</span></a>
											</area>
										</div>
										<!-- //button-display -->
									</div>
				...
				<div class="local-util">
					<div class="button-display module-a style-b type-d">
						...
						<?
							if ($rs_va_state == 0) {
						?>
							<button onClick="js_delete()" class="btn module-b style-a type-fill normal-03 x-large flex"><span class="btn-text">삭제</span></button>
							<a href="expense_write.php?mode=S&ex_no=<?=$ex_no?>" class="btn module-b style-a type-fill accent-02 x-large flex"><span class="btn-text">수정</span></a>
						<?
							} else {
						?>
