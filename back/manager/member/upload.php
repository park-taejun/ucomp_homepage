...
#====================================================================
	...
	#====================================================================
		...
		$chk_str = trim($objWorksheet->getCell('AB' . 1)->getValue());
...
<?
		...
		for ($i = 2 ; $i <= $maxRow ; $i++) {
			...
			$req_day				= trim($objWorksheet->getCell('AB' . $i)->getValue());
			...
			$result =  insertTempMember($conn, $arr_data);
...
?>	
<script language="javascript">
	location.href =  'upload.php?mode=L&temp_no=<?=$file_nm?>';
</script>
<?
		mysql_close($conn);
		exit;
...
?>
		...
		<div id="temp_scroll">
			...
			<table cellpadding="0" cellspacing="0" border="0" style="width:3365px">
					<colgroup>
						<col width="35">
						<col width="100">	<!-- 비고 -->
						<col width="70">	<!-- 이름 -->
					...
					<?
						...
						if (sizeof($arr_rs) > 0) {
							...
							for ($j = 0 ; $j < sizeof($arr_rs); $j++) {
								...
								// 조직정보 가지고 오기
								$M_ORGANIZATION = searchGroupCd($conn, $PARTY, $SIDO, $M_ORGANIZATION);

								if ($M_ORGANIZATION <> "") {
									$result = updateOrganization($conn, $M_ORGANIZATION, $SEQ_NO);
								}

								if (($REQ_DAY == "5") || ($REQ_DAY == "25") || ($REQ_DAY == "0")) {
									//
								} else {
									$err_str .=  "출금일 오류 2,25, 0 만 입력 가능 ";
								}
					...
					?>
						<tr>
							...
							<td style="text-align:center"><?=$REQ_DAY?></td>
						</tr>
					<?			
										$err_str = "";
									}
							...
							?> 
								<tr>
									<td align="center" height="50"  colspan="30">데이터가 없습니다. </td>
								</tr>
							<? 
								}
							?>
