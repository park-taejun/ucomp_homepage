						...
						<table cellpadding="0" cellspacing="1" border="0" width="95%"> 
							...
							<?
								...
								if ($TotalArticle > $ListArticle) {
									if ($page != 1)
										echo "[<a href='".$PHP_SELF."?page=1&code_id=$code_id&sYYYY=$sYYYY&sidxfield=$idxfield&qry_str=$qry_str'>맨앞</a>]";
									
									// 이전페이지
									if (($TotalArticle + 1) > ($ListArticle * $PageScale)) {
										$PrevPage = ($Scale - 1) * $PageScale;
										if ($PrevPage >= 0)
											echo "&nbsp;[<a href='".$PHP_SELF."?page=".($PrevPage + 1)."&idxfield=$idxfield&code_id=$code_id&sYYYY=$sYYYY&qry_str=$qry_str'>이전".$PageScale."개</a>]";
									}
									echo "&nbsp;";
									// 페이지 번호
									...
									if ($TotalArticle > (($Scale + 1) * $ListArticle * $PageScale)) {
										...
										echo "[<a href='".$PHP_SELF."?page=".$NextPage."&code_id=$code_id&sYYYY=$sYYYY&idxfield=$idxfield&qry_str=$qry_str'>이후".$PageScale."개</a>]";
									}
										
									if ($page != $TotalPage)
										echo "&nbsp;[<a href='".$PHP_SELF."?page=".$TotalPage."&code_id=$code_id&sYYYY=$sYYYY&idxfield=$idxfield&qry_str=$qry_str'>맨뒤</a>]&nbsp;&nbsp;";
								} else 
									echo "&nbsp;[1]&nbsp;";	
							?>
								</td>
