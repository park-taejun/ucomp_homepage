...
#=====================================================================
	...
	$className				= $_POST['className']!=''?$_POST['className']:$_GET['className'];

	$search_field			= $_POST['search_field']!=''?$_POST['search_field']:$_GET['search_field'];
	$search_str				= $_POST['search_str']!=''?$_POST['search_str']:$_GET['search_str'];

	$y = "202206"; //본부, 부서를 위한 년도, $year을 사용중이어서.

	if($className == ""){
		$className = "datepicker-inline style-b";
	}
	...
	<div id="page">
		...
		<div class="page-navi">
			<ul class="navi-list">
				...
				<li class="navi-item current" title="선택 됨"><span class="navi-name">근무현황</span></li>
				...
				<div class="local-body">
					...
					<div id="content">
						...
						<div class="content-head">
							...
							<!-- data-finder -->
							<form class="data-finder" id="searchBar" name="frm">
						...
						<div class="content-body">
							<!-- datepicker -->
							<div class="<?=$className?>" id="datepickerInline" data-bui-expand="workingStatus"></div>
							<form name="frm_date" method="post">
							<input type="hidden" name="selectedDate" value="<?=$selectedDate?>">
							<input type="hidden" name="className" value="<?=$className?>">
							<input type="hidden" name="search_field" value="<?=$search_field?>">
							<input type="hidden" name="search_str" value="<?=$search_str?>">
							<input type="hidden" name="con_headquarters_code" value="<?=$con_headquarters_code?>">
									...
									<div class="data-display module-b style-a type-a normal-03 small">
										...
										<?
											...
											for($i = 0 ; $i < sizeof($arr_rs); $i++) {
												...
												if($search_str <> "") {
													if($search_str <> $ADM_NAME) {
														continue;
													}
												}
												if($search_field == "HEADQUARTERS_2022") {
													if(($con_headquarters_code <> $HEADQUARTERS_CODE) and ($con_headquarters_code <> "")) {
														continue;
													}
												}
												if($search_field == "DEPT_2022") {
													if(($con_dept_code <> $DEPT_CODE) and ($con_dept_code <> "")) {
														continue;
													}
												}
										?>
											...
											<li class="data-item">
												<div class="data-wrap">
													<div class="data-head"><a href="javascript:void(0)" class="data-name symbol-rtl-fill-circle-more" onclick="js_open_modal(<?=$SEQ_NO?>, '<?=$selectedDate?>')"><?=$ADM_NAME?></a></div>
...
<div class="content-popup module-a style-a type-a small" id="schedule" data-bui-toggle="contentsPopup">
	<div class="popup-page-body">
		...
		<div class="popup-local">
			...
			<!-- popup-local-body -->
			<div class="popup-local-body">
				<div class="popup-content" id="pop-content">
					<!-- popup-content-body -->
					<div class="popup-content-body">
						<!-- goods-display -->
									...
									<div class="goods-wrap">
										<div class="goods-inform">
											<div class="goods-head">
												<p class="goods-title">
													<span class="goods-name" id="goods-name">홍길순</span>
													<span class="goods-position" id="goods-position">선임</span>
													<span class="goods-department" id="goods-department">경영기획팀</span>
													<span class="goods-team" id="goods-team">재무/회계</span>
												</p>
											</div>
										</div>
									</div>
...
<script type="text/javascript">
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		const datepicker = new AirDatepicker('#datepickerInline', {
			...
			onSelect: function(date, formattedDate, datepicker) {
				...
				frm.className.value = $('#datepickerInline').attr("class");
				frm.selectedDate.value = dd;
				frmsearch = document.frm;
				if ( frmsearch.search_field.value != 'ADM_NAME') {
					frmsearch.search_field.value = '<?=$search_field?>';
					frmsearch.search_str.value = '';
					//frm.con_headquarters_code.value = '<?=$con_headquarters_code?>';
					//frm.con_dept_code.value = '<?=$con_dept_code?>';
				} else {
					frmsearch.search_field.value = '<?=$search_field?>';
					frmsearch.search_str.value = '<?=$search_str?>';
				}
	...
	function js_open_modal(seq_no, selectedDate) {

	//contentsPopup.active('schedule');

		var request = $.ajax({
			url:"/m/vacation/vacation_modal.php",
			type:"POST",
			data:{seq_no:seq_no, selectedDate:selectedDate},
			dataType:"html",
