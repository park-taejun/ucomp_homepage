<?
	...
	function makeBannerSelectBoxOnChange($db,$pcode,$objname,$size,$str,$val,$checkVal) {
	...
	function makeSelectBoxOnChange($db,$pcode,$objname,$size,$str,$val,$checkVal) {
		...
		$query = "	SELECT 
						DCODE, DCODE_NM
					FROM TBL_CODE_DETAIL WHERE 1 = 1 AND DEL_TF = 'N' AND USE_TF = 'Y'  AND PCODE = 'STORY_TYPE' ";
	...
	function makeBannerSelectBoxAllOnChange($db,$pcode,$objname,$size,$str,$val,$checkVal) {

		$query = "SELECT DCODE, DCODE_NM
								FROM TBL_CODE_DETAIL WHERE 1 = 1 AND DEL_TF = 'N' AND PCODE = 'BANNER' ";
	...
	function makeAdminGroupSelectBox($db, $objname,$size,$str,$val,$checkVal) {
		...
		$query = "SELECT GROUP_NO, GROUP_NAME
								FROM TBL_HOME_ADMIN_GROUP WHERE 1 = 1 AND DEL_TF = 'N' AND USE_TF = 'Y' ";
	...
	function getListAdminGroupMenu($db, $group_no ) {   
		if ($group_no == "1") {
			$query = "SELECT CONCAT(A.MENU_SEQ01,A.MENU_SEQ02,A.MENU_SEQ03) as SEQ, A.MENU_CD, A.MENU_NAME, A.MENU_URL, 
					...
					FROM TBL_HOME_ADMIN_MENU A, TBL_HOME_ADMIN_MENU_RIGHT B 
	...
	function getListAdminGroupMenu_ptj($db, $group_no, $rs_leader_yn,  $rs_level ) {
		 
		 
			if ( $rs_leader_yn == "Y" && $rs_level == "4" ) {
				$query = "SELECT CONCAT(A.MENU_SEQ01,A.MENU_SEQ02,A.MENU_SEQ03) as SEQ, A.MENU_CD, A.MENU_NAME, A.MENU_URL, 
										 B.READ_FLAG, B.REG_FLAG, B.UPD_FLAG, B.DEL_FLAG, B.FILE_FLAG, A.MENU_RIGHT 
...
<?
  ...
  // left outer join TBL_CODE_DETAIL B on A.DEPT_CODE = B.DCODE AND B.PCODE = 'DEPT' 
	...
	function makeEmpSelectBox202206($db,$objname,$size,$str,$val,$checkVal) {
  ...
  // left outer join TBL_CODE_DETAIL B on A.DEPT_CODE = B.DCODE AND B.PCODE = 'DEPT' 
	...
	function getEmpInfo($db, $adm_no) {
		...
		$query = " SELECT A.ADM_ID, B.DCODE_NM, A.ADM_NO, A.ADM_NAME FROM TBL_ADMIN_INFO A left outer join TBL_CODE_DETAIL B on A.DEPT_CODE = B.DCODE AND B.PCODE = 'DEPT_2022' 
								WHERE A.COM_CODE = '1'
									AND A.DEL_TF = 'N'
									AND A.ADM_NO = '$adm_no' ";
	...
	function getEmpInfoName($db, $adm_no) {
...
//승인상태 함수
	...
	function selectVaState2022($db, $va_state){
		switch ($va_state){
			case "0" : $record = "진행 중";
			break;
			case "1" : $record = "승인";
