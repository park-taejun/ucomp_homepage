...
#===============================================================
	...
	$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], "CMS 등록오류 리스트조회", "List");
				...
				<legend class="conTitle"><?=$p_menu_name?></legend>
							...
							<tr class="last">
								...
								<td>
									...
									<select name="search_field" style="width:84px;">
										...
										<option value="CMS_RESULT_MSG" <? if ($search_field == "CMS_RESULT_MSG") echo "selected"; ?> >오류정보</option>
					...
					?>
							...
							<tr <? if ($j == (sizeof($arr_rs)-1)) {?> class="last" <? } ?> >
								<td><?=$rn?></td>
								<td><a href="javascript:js_modify('<?=$M_NO?>');"><?=$M_ID?></a></td>
								<td><a href="javascript:js_modify('<?=$M_NO?>');"><?=$M_NAME?></a></td>
								<td><?=$SIDO?></td>
								<td>
								<?
									if($M_SEX=="1"){
