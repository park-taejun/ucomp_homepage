...
#=====================================================================
	...
	require "../_classes/biz/week/week.php";
...
<?
	...
	if ($menu == "brochure") {
		...
		$str_path = $g_physical_path."upload_data/brochure/"; //브로셔 폴더 권한 오류로 폴더 변경 2022-06-03
		
		$filename_rnm	= trim($arr_rs[0]["FILE_NM"]); 
		$filename_nm	= trim($arr_rs[0]["FILE_RNM"]); 
		
		$result_view = updateViewCntBrochure($conn, $file_no);
	...
	if ($menu == "request") {
		...
		$filename_rnm			= trim($arr_rs[0]["FILE_NM"]); 
		$filename_nm			= trim($arr_rs[0]["FILE_RNM"]); 
		$str_path = $g_physical_path."upload_data/request/";
			
	}	

	if ($menu == "weekreport") {
		
		$arr_rs = selectWeekReport($conn, $seq_no);

		$filename_rnm			= trim($arr_rs[0]["WE_RFILE"]); 
		$filename_nm			= trim($arr_rs[0]["WE_FILE"]); 
		$str_path = $g_physical_path."upload_data/weekreport/";
