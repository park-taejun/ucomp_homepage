...
(function (root, factory) {
	...
	var Constructor = function(selector, options) {
		...
		function focusin(settings, toggleThis) {
			toggleThis.toggleTarget.setAttribute('tabindex', '1');
		};
 ...
 (function (root, factory) {
	...
	var Constructor = function (selector, options) {
		...
		publicAPIs.setup = function() {
			// Variables
			var tabDisplay = document.querySelectorAll(selector);
			if (!tabDisplay) return;

			
			Array.prototype.forEach.call(tabDisplay, function(tabList) {
				console.log(tabList);
				

				Array.prototype.forEach.call(tabList.querySelectorAll(settings.tabItem), function(tabItem, index) {
					const tabName = tabItem.querySelector(settings.tabName);
					...
					if (settings.initial == index) {
						tabItem.classList.add(settings.activeClass);
						tabItem.setAttribute('title', settings.activeTitle);
					...
					tabName.addEventListener('click', function(e) {
						...
						e.currentTarget.parentElement.setAttribute('title', settings.activeTitle);
