...
#====================================================================
	...
	$arr_rs = paymentStatsAsArea($conn, $start_date, $end_date, $sel_pay_type, $sel_area_cd, $sel_party, $sel_pay_state, $sel_pay_reason, $group_cd);

	$result_log = insertUserLog($conn, 'admin', $_SESSION['s_adm_id'], $_SERVER['REMOTE_ADDR'], "지역별 당비 현황  조회", "Stats");
...
<input type="hidden" name="group_cd" id="group_cd"/>
				...
				<legend class="conTitle"><?=$p_menu_name?></legend>
					...
					<div class="expArea">
						<ul class="fLeft">
							<li class="total">총 당비 <?=number_format($tot_cnt)?> 원</li> 
						</ul>
						<p class="fRight">
							<? if ($sPageRight_F == "Y") { ?>
					...
					?>
							<tr <? if ($j == (sizeof($arr_rs)-1)) {?> class="last" <? } ?> >
								...
								<td style="text-align:left">
									납부 당비 : <img src='../images/etc/red.gif' width='<?=$img_width?>' height='10'> (<?=number_format($IN_COUNT)?> 원)
									<div class="sp5"></div>
									환불 당비 : <img src='../images/etc/sur.gif' width='<?=$out_img_width?>' height='10'> (<?=number_format($OUT_COUNT)?> 원)
								</td>
							</tr>
					<?
							}
