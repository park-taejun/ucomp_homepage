...
//$_SESSION['s_adm_no']="25";
	...
	if ($LEADER_YN <> "Y") {
		header("location:/m/expense/expense_list.php");
		exit;
	}
		...
		if(chk==""){
			...
			<? } ?>
				if (re==true) {
					...
					frm.method = "get";
					frm.action = "approval_list_dml.php";
					frm.submit();
				}
		}
	}
			...
			<div id="local">
				...
				<div class="local-head">
					...
					<div class="local-navi">
						<ul class="navi-list">
							<li class="navi-item"><a class="navi-name" href="vacation_approval_list.php">연차결재</a></li>
				...
				<div class="local-body">
					...
					<div id="content">
						...
						<div class="content-head">
							...
							<div class="tab-display module-c style-a type-d medium">
								...
								<? if (($LEADER_YN == "Y") || ($s_adm_no == 1) || ($s_adm_no == 178) || ($s_adm_no == 4) || ($s_adm_no == 43) || ($s_adm_no == 44)) { ?> 
									<li class="tab-item current" title="현재 선택된 항목"><span class="tab-name">지출결재</span></li>
									<li class="tab-item"><a class="tab-name" href="approval_all_list.php">모든요청</a></li>
								<? }
									 if (($my_level <> 0) && ($s_adm_no <> 1 ) && ($s_adm_no <> 178) && ($s_adm_no <> 4 ) && ($s_adm_no <> 43 ) && ($s_adm_no <> 44 )) { ?>
									<li class="tab-item"><a class="tab-name" href="/m/expense/expense_list.php">나의요청</a></li>
								<? } ?>
								</ul>
							</div>
							<!-- //tab-display -->
						...
						<? 
							$state = $r; //신청
							$arr_rs   = listApproval($conn, $s_adm_no, $state, $search_field, $search_str); 
						?>
...
<?
	if (sizeof($arr_rs) > 0) {
		...
		for($j = 0 ; $j < sizeof($arr_rs) ; $j++){
				...
				$EX_TYPE					= Trim($arr_rs[$j]["EX_TYPE"]);
				$EX_TITLE					= Trim($arr_rs[$j]["EX_TITLE"]);
	...
	<div class="post-item">
		<div class="post-wrap">
			...
			<? if (($s_adm_no <> 1) && ($s_adm_no <> 178) && ($s_adm_no <> 4) && ($s_adm_no <> 43) && ($s_adm_no <> 44) && ($r == "")) {?>
				<span class="form module-a style-c type-a medium checkbox"><input type="checkbox" name="chk[]" value="<?=$EX_NO?>" class="form-elem" title="항목 선택"></span>
			...
			<div class="post-inform">
				...
				<div class="post-head">
					<p class="post-title">
						<a class="post-name" onClick="js_view('<?=$EX_NO?>')" ><?=$EX_TITLE?> - <?=$EX_TOTAL_PRICE?>원</a>
					</p>
				</div>
				<div class="post-data">
					<ul class="data-list">
