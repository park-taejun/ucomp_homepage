				...
				<legend class="conTitle"><?=$p_menu_name?></legend>
						...
						<tbody>
							...
							<tr>
								...
								<td>
									...
									<?= makeSelectBox($conn,"CMS_PAY_DAY","sel_pay_day", "100","출금일 선택", "",$sel_pay_day);?>
					...
					<div class="expArea">
						...
						<p class="fRight">
							...
							약정일 : 
							<?=makeRadioBox($conn,"CMS_PAY_DAY","req_date","")?>

							출금일 :
							<input type="text" name="sel_pick_date" id="sel_pick_date" class="date" value="" style="width:100px;border:1px solid #dfdfdf;" readonly="1"> 
							<input type="button" name="aa" value=" 예상 출금액 " style="border:1px solid #dfdfdf;cursor:pointer;height:25px;" onclick="js_pre_cal();">
						</p>
					</div>
					<div id="pre_amout">
					</div>
			...
			<div id="bbspgno">
					...
					<?
						...
						if (sizeof($arr_rs) > 0) {
							#$search_field		= trim($search_field);
							#$search_str			= trim($search_str);
							$strParam = $strParam."&nPageSize=".$nPageSize."&search_field=".$search_field."&search_str=".$search_str."&sel_pay_yyyy=".$sel_pay_yyyy."&sel_pay_mm=".$sel_pay_mm."&sel_pay_day=".$sel_pay_day."&sel_area_cd=".$sel_area_cd."&sel_pay_type=".$sel_pay_type."&sel_party=".$sel_party."&sel_pay_state=".$sel_pay_state."&Ngroup_cd=".$Ngroup_cd;
