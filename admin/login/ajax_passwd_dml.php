...
#====================================================================
	...
	$ex_passwd		= trim($_POST['ex_passwd']);
	...
	if ($mode == "S") {
		...
		if ($rs_adm_no == "") {
			...
			$str_newpasswd = encrypt($key, $iv, $new_passwd);
			$str_resetpasswd = decrypt($key, $iv, $rs_passwd);	
			
			echo $str_resetpasswd;

			if ($ex_passwd == $str_resetpasswd) {
				$result = "0";
				$str_result = "";
				
				$str_update = updatePassword($conn, $rs_adm_no, $str_newpasswd);
				
			} else {
				$result = "2";
				$str_result = "기존 비밀번호를 다시 확인 해주세요.";
			}
		}
